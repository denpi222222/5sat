...
export const SocialSidebar = React.memo(function SocialSidebar() {
  ...
  useEffect(() => {
    ...
    const showIcons = () => {
      if (showCount >= 3 || isManuallyClosed) return; // Max 3 times per session or if manually closed
    ...
    if (isMobile) {
      ...
      const scheduleNextShow = () => {
        if (showCount < 3 && !isManuallyClosed) {
  ...
  }, [isMobile, showCount, isManuallyClosed]);
